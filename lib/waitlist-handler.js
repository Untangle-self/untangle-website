...
export async function handleWaitlistSubmission(input, env) {
  ...
  if (redis) {
    const duplicate = await isDuplicateEmail(redis, normalizedEmail)
    console.log('[waitlist:redis:diag] isDuplicateEmail() result:', duplicate)
    if (duplicate) {
      return { status: 200, body: { ...WAITLIST_DUPLICATE } }
    }

    const reserved = await reserveEmail(redis, normalizedEmail)
    console.log('[waitlist:redis:diag] reserveEmail() result:', reserved)
    if (!reserved) {
      return { status: 200, body: { ...WAITLIST_DUPLICATE } }
    }
  } else {
    console.log('[waitlist:redis:diag] handler redis block skipped (no client)')
  }
